...
function adminButtons(ticketNumber) {
  return Markup.inlineKeyboard([
    ...
    [
      Markup.button.callback(
        "✅ Successful",
        `success_${ticketNumber}`
      ),

      Markup.button.callback(
        "🏁 Resolve",
        `resolve_${ticketNumber}`
      ),
    ],
  ]);
}
...
async function showTicketsByStatus(
  ctx,
  status,
  emptyText
) {
